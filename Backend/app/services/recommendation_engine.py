...
2. Distance (25pts):    Closer to user's classes scores highest
3. Preferences (15pts): Matches user's preferred parking type
4. Major (10pts):       Spot is on the same campus as the user's major
5. Verified (10pts):    Verified spots are more trustworthy
"""
...
class RecommendationEngine:
    ...
    event_lon: float | None = None,
        ...
        for spot in spots:
            scored_spot = self._score_spot(
                spot, user, user_lat, user_lon, event_lat, event_lon
            )
    ...
    def _score_spot(
        ...
        total = cost_score + distance_score + preference_score + major_score + verified_score + event_score
    ...
    def _score_preferences(self, spot: ParkingSpot, user: User) -> float:
        """
        Weight = 15pts
        ...
        Example: user prefers "Parking Garage,Surface Lot"
                 spot is "Parking Garage" → 15pts
        """
        if not user.preferred_parking_types or not spot.parking_type:
            return 0.0
    ...
    def _score_major(self, spot: ParkingSpot, user: User) -> float:
        """
        Weight = 10pts
        ...
        """
        if not user.major_category or not spot.campus_location:
            return 0.0
    ...
    def _score_verified(self, spot: ParkingSpot) -> float:
        ...
        """
        return 10.0 if spot.is_verified else 0.0
